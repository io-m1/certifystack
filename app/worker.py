...
def generate_certificate(job_id: int, certificate_id: str, draft_id: int = None, **kwargs):
    ...
    with app.app_context():
        ...
        if cert.status in [CertificateStatus.READY_FOR_DISPATCH, CertificateStatus.QUEUED_FOR_DISPATCH, CertificateStatus.DISPATCHING, CertificateStatus.SENT]:
        ...
        try:
            ...
            # Shared render seam: SVG pipeline for uploaded templates, the
            # MedLocum designed certificate for template-less gateways.
            from app.domain.certificates.service import render_certificate_bytes

            pdf_bytes = render_certificate_bytes(
                user, cert_type, org, issued_at=user.sent_at or datetime.utcnow()
            )
            if not pdf_bytes:
                raise RuntimeError("Certificate rendering produced no PDF bytes")
