...
def _cairosvg():
    """Import cairosvg lazily: the native cairo library is only present in
    deployment images (Dockerfile installs it). Hosts without it can still
    run the app — MLJ-designed and legacy-overlay rendering don't need it —
    and only actual SVG conversion raises."""
    try:
        import cairosvg
        return cairosvg
    except OSError as e:
        raise ConversionError(f'cairosvg unavailable (native cairo missing): {e}')
...
def svg_to_pdf_bytes(svg_path: str, dpi: int = 300) -> bytes:
    ...
    try:
        ...
        pdf_data = _cairosvg().svg2pdf(bytestring=svg_bytes, dpi=dpi)
